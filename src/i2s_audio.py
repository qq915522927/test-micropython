...
def play_audio_from_http_stream(response):
    """
    Play wave from http stream
    """
    # Read the WAV file header
    riff, size, fformat = struct.unpack('<4sI4s', response.raw.read(12))
    fmt_header, fmt_size, audio_format, channels, samplerate, byterate, \
    block_align, bits_per_sample = struct.unpack('<4sIHHIIHH', response.raw.read(24))
    print("声道数：", channels)
    print("采样宽度：", bits_per_sample)
    print("采样频率：", samplerate)
    # 跳过文件的开头的44个字节，直到数据段的第1个字节
